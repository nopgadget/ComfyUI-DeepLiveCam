...
from .gettext import LanguageManager
import platform
...
img_ft, vid_ft = globals.file_types
...
def save_switch_states():
    switch_states = {
        ...
        "keep_audio": globals.keep_audio,
        "keep_frames": globals.keep_frames,
        "many_faces": globals.many_faces,
        "map_faces": globals.map_faces,
        "color_correction": globals.color_correction,
        "nsfw_filter": globals.nsfw_filter,
        "live_mirror": globals.live_mirror,
        "live_resizable": globals.live_resizable,
        "fp_ui": globals.fp_ui,
        "show_fps": globals.show_fps,
        "mouth_mask": globals.mouth_mask,
        "show_mouth_mask_box": globals.show_mouth_mask_box,
    }
...
def load_switch_states():
    try:
        ...
        globals.keep_fps = switch_states.get("keep_fps", True)
        globals.keep_audio = switch_states.get("keep_audio", True)
        globals.keep_frames = switch_states.get("keep_frames", False)
        globals.many_faces = switch_states.get("many_faces", False)
        globals.map_faces = switch_states.get("map_faces", False)
        globals.color_correction = switch_states.get("color_correction", False)
        globals.nsfw_filter = switch_states.get("nsfw_filter", False)
        globals.live_mirror = switch_states.get("live_mirror", False)
        globals.live_resizable = switch_states.get("live_resizable", False)
        globals.fp_ui = switch_states.get("fp_ui", {"face_enhancer": False})
        globals.show_fps = switch_states.get("show_fps", False)
        globals.mouth_mask = switch_states.get("mouth_mask", False)
        globals.show_mouth_mask_box = switch_states.get(
...
def create_root(start: Callable[[], None], destroy: Callable[[], None]) -> ctk.CTk:
    ...
    root.title(
        f"{metadata.name} {metadata.version} {metadata.edition}"
    )
    root.configure()
    ...
    keep_fps_value = ctk.BooleanVar(value=globals.keep_fps)
    keep_fps_checkbox = ctk.CTkSwitch(
        ...
        cursor="hand2",
        command=lambda: (
            setattr(globals, "keep_fps", keep_fps_value.get()),
            save_switch_states(),
        ),
    )
    keep_fps_checkbox.place(relx=0.1, rely=0.6)

    keep_frames_value = ctk.BooleanVar(value=globals.keep_frames)
    keep_frames_switch = ctk.CTkSwitch(
        ...
        cursor="hand2",
        command=lambda: (
            setattr(globals, "keep_frames", keep_frames_value.get()),
            save_switch_states(),
        ),
    )
    keep_frames_switch.place(relx=0.1, rely=0.65)

    enhancer_value = ctk.BooleanVar(value=globals.fp_ui["face_enhancer"])
    ...
    keep_audio_value = ctk.BooleanVar(value=globals.keep_audio)
    keep_audio_switch = ctk.CTkSwitch(
        ...
        cursor="hand2",
        command=lambda: (
            setattr(globals, "keep_audio", keep_audio_value.get()),
            save_switch_states(),
        ),
    )
    keep_audio_switch.place(relx=0.6, rely=0.6)

    many_faces_value = ctk.BooleanVar(value=globals.many_faces)
    many_faces_switch = ctk.CTkSwitch(
        ...
        cursor="hand2",
        command=lambda: (
            setattr(globals, "many_faces", many_faces_value.get()),
            save_switch_states(),
        ),
    )
    many_faces_switch.place(relx=0.6, rely=0.65)

    color_correction_value = ctk.BooleanVar(value=globals.color_correction)
    color_correction_switch = ctk.CTkSwitch(
        ...
        cursor="hand2",
        command=lambda: (
            setattr(globals, "color_correction", color_correction_value.get()),
            save_switch_states(),
        ),
    )
    color_correction_switch.place(relx=0.6, rely=0.70)

    #    nsfw_value = ctk.BooleanVar(value=globals.nsfw_filter)
    #    nsfw_switch = ctk.CTkSwitch(root, text='NSFW filter', variable=nsfw_value, cursor='hand2', command=lambda: setattr(globals, 'nsfw_filter', nsfw_value.get()))
    #    nsfw_switch.place(relx=0.6, rely=0.7)

    map_faces = ctk.BooleanVar(value=globals.map_faces)
    map_faces_switch = ctk.CTkSwitch(
        ...
        cursor="hand2",
        command=lambda: (
            setattr(globals, "map_faces", map_faces.get()),
            save_switch_states(),
    ...
    show_fps_value = ctk.BooleanVar(value=globals.show_fps)
    show_fps_switch = ctk.CTkSwitch(
        ...
        cursor="hand2",
        command=lambda: (
            setattr(globals, "show_fps", show_fps_value.get()),
            save_switch_states(),
        ),
    )
    show_fps_switch.place(relx=0.6, rely=0.75)

    mouth_mask_var = ctk.BooleanVar(value=globals.mouth_mask)
    mouth_mask_switch = ctk.CTkSwitch(
        root,
        text=_("Mouth Mask"),
        variable=mouth_mask_var,
        cursor="hand2",
        command=lambda: setattr(globals, "mouth_mask", mouth_mask_var.get()),
    )
    mouth_mask_switch.place(relx=0.1, rely=0.55)

    show_mouth_mask_box_var = ctk.BooleanVar(value=globals.show_mouth_mask_box)
    show_mouth_mask_box_switch = ctk.CTkSwitch(
        ...
        command=lambda: setattr(
            globals, "show_mouth_mask_box", show_mouth_mask_box_var.get()
        ),
    )
...
def analyze_target(start: Callable[[], None], root: ctk.CTk):
    ...
    if globals.map_faces:
        ...
        if len(globals.source_target_map) > 0:
            create_source_target_popup(start, root, globals.source_target_map)
        else:
...
def select_source_path() -> None:
    ...
    if is_image(source_path):
        ...
        image = render_image_preview(globals.source_path, (200, 200))
...
def swap_faces_paths() -> None:
    ...
    target_path = globals.target_path

    if not is_image(source_path) or not is_image(target_path):
        return

    globals.source_path = target_path
    globals.target_path = source_path

    RECENT_DIRECTORY_SOURCE = os.path.dirname(globals.source_path)
    RECENT_DIRECTORY_TARGET = os.path.dirname(globals.target_path)

    PREVIEW.withdraw()

    source_image = render_image_preview(globals.source_path, (200, 200))
    source_label.configure(image=source_image)

    target_image = render_image_preview(globals.target_path, (200, 200))
...
def select_target_path() -> None:
    ...
    if is_image(target_path):
        ...
        RECENT_DIRECTORY_TARGET = os.path.dirname(globals.target_path)
        image = render_image_preview(globals.target_path, (200, 200))
...
def select_output_path(start: Callable[[], None]) -> None:
    ...
    output_path = ctk.filedialog.askdirectory(title=_("Select directory"))
    if output_path:
        globals.output_path = output_path
        RECENT_DIRECTORY_OUTPUT = output_path
        # Both source and target paths must be selected
        if is_image(globals.source_path) and (is_image(globals.target_path) or is_video(globals.target_path)):
            start()


def check_and_ignore_nsfw(target, destroy: Callable = None) -> bool:
    try:
        from .predicter import predict_image, predict_video
    except ImportError:
        update_status("Error importing NSFW filter, NSFW detection disabled")
        return False
    
    # determine if the target is an image or video
    try:
        if is_video(target):
            nsfw = predict_video(target)
        else:
            nsfw = predict_image(target)
        
        # If NSFW content detected
        if nsfw:
            update_status(
                "NSFW content detected! Processing aborted. De-activate filter in settings."
            )
            return True
    except Exception as e:
        update_status(f"Error in NSFW detection: {str(e)}")
    return False
...
def toggle_preview() -> None:
    if PREVIEW.state() == "normal":
        PREVIEW.withdraw()
    elif globals.source_path and globals.target_path:
        init_preview()
        update_preview()
...
def init_preview() -> None:
    ...
    if is_video(globals.target_path):
        video_frame_total = get_video_frame_total(globals.target_path)
...
def update_preview(frame_number: int = 0) -> None:
    if globals.source_path and globals.target_path:
        update_status("Processing...")
        temp_frame = get_video_frame(globals.target_path, frame_number)
        if globals.nsfw_filter and check_and_ignore_nsfw(temp_frame):
            return
        for frame_processor in get_frame_processors_modules(
                globals.frame_processors
        ):
            temp_frame = frame_processor.process_frame(
                get_one_face(cv2.imread(globals.source_path)), temp_frame
            )
...
def webcam_preview(root: ctk.CTk, camera_index: int):
    ...
    if not globals.map_faces:
        if globals.source_path is None:
        ...
        create_source_target_popup_for_webcam(
            root, globals.source_target_map, camera_index
        )
...
def create_webcam_preview(camera_index: int):
    ...
    while True:
        ...
        if not globals.map_faces:
            if source_image is None and globals.source_path:
                source_image = get_one_face(cv2.imread(globals.source_path))

            for frame_processor in frame_processors:
                if frame_processor.NAME == "DLC.FACE-ENHANCER":
                    if globals.fp_ui["face_enhancer"]:
